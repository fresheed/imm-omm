...
Section OCamlMM_TO_IMM_S_PROG.
  ...
  Proof.
    ...
    { intros ACB. desc. red in COMP. desc.
      ...
      { congruence. }
      forward eapply BLOCK_INDEX as [b BLOCK]; eauto.
      unfold sublist in PROG_BLOCK. 
      (* problem: block can appear in multiple positions*)
      foobar. 
      ...
      cut (exists b : nat, Some block = nth_error BPI b).
  ...
  Proof.
    ...
    { forward eapply (steps_split (step tid) (length block) (n - length block)) as SPLIT.
      ...
      (* specialize (SPLIT (bst2st bst1) (bst2st bst2)). *)
      ...
      { congruence. }
      (* (H (n - length block) LEN' st' (bst2st bst2) tid NEXT_STEPS COMP' bst' BST' bst2). *)
...
Section CompilationCorrectness.
  ...
  Lemma thread_execs_helper: exists GO,
      ...
      ⟪ SAME_INIT: forall e : actid, is_init e -> E GO e <-> E GI e⟫ /\
      ⟪ SAME_CO: co GI ≡ co GO⟫ /\
  ...
  Lemma GO_exists: exists GO,
      ...
      same_behavior GO GI. 
  Proof.
    pose proof thread_execs_helper. desc.
    exists GO. split.
    { red. split; auto. 
    ...
    { intros EGOx. red.
      ...
      forward eapply RESTR_SIM as [OTHREADEXEC SBL]; eauto.
      split. 
    ...
    { assert (exists PIi, Some PIi = IdentMap.find thread ProgI) by admit.
      ...
      forward eapply RESTR_SIM as [OEXEC SBL]; eauto.
      destruct SBL. apply (set_equiv_exp H2).
